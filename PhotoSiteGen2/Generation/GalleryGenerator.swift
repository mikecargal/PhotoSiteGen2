...
struct GalleryGenerator {
    ...
    func distanceAsText(subjectDistance: Double?) -> String {
        ...
        if subjectDistance == 4_294_967_295 || subjectDistance == 2_147_483_647
        {
            return "∞"
        }
        ...
        if subjectDistance > 100 {
            return
                "\(String(format: "%.0f", ft)) ft (\(String(format: "%.0f", subjectDistance)) m)"
        }
        if subjectDistance > 10 {
            return
                "\(String(format: "%.1f", ft)) ft (\(String(format: "%.1f", subjectDistance)) m)"
        }
        if ft > 1 {
            return
                "\(String(format: "%.2f", ft)) ft (\(String(format: "%.2f", subjectDistance)) m)"
        }
        return
            "\(String(format: "%.1f", ft*12)) in (\(String(format: "%.3f", subjectDistance)) m)"
    }
}
